...
export async function POST(req: NextRequest, ctx: { params: Promise<{ resource: string }> }) {
  ...
  try {
    const auth = authorize(req, resource, "POST"); if (!auth) return Response.json({ error: "Forbidden" }, { status: 403 }); const body = await req.json();
    if (!body || typeof body !== "object" || !body.values || typeof body.values !== "object") return Response.json({ error: "values object is required" }, { status: 400 });
    const row: any = buildRow(entity, body.values);
    ...
    const result: any = await db.insert(table).values(row).returning(); const inserted = Array.isArray(result) ? result[0] : result;
    await audit(`${entity.singular} created`, firstLabel(entity, inserted), "", auth.user, getClientIp(req)); return Response.json(redactValue(inserted));
  } catch (e: any) { if (e?.status) return Response.json({ error: e.message }, { status: e.status }); return Response.json({ error: String(e?.message || "Create failed") }, { status: 500 }); }
}
