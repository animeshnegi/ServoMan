...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ resource: string; id: string }> }) {
  ...
  try { const auth = authorize(req, resource, "PATCH"); if (!auth) return Response.json({ error: "Forbidden" }, { status: 403 }); const numericId = parseId(id); if (!numericId) return Response.json({ error: "Invalid id" }, { status: 400 }); const body = await req.json(); if (!body || typeof body !== "object" || !body.values || typeof body.values !== "object") return Response.json({ error: "values object is required" }, { status: 400 });
    const existingRows: any[] = await db.select().from(table).where(eq(table.id, numericId)).limit(1); if (!existingRows[0]) return Response.json({ error: "Not found" }, { status: 404 }); const row: any = buildRow(entity, body.values);
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ resource: string; id: string }> }) {
  const { resource, id } = await ctx.params; const table: any = tableMap[resource]; const entity = ENTITY_MAP[resource]; if (!table || !entity) return Response.json({ error: "Unknown resource" }, { status: 404 });
  try { const auth = authorize(req, resource, "DELETE"); if (!auth) return Response.json({ error: "Forbidden" }, { status: 403 }); const numericId = parseId(id); if (!numericId) return Response.json({ error: "Invalid id" }, { status: 400 }); const existing = await db.select().from(table).where(eq(table.id, numericId)).limit(1); if (!existing[0]) return Response.json({ error: "Not found" }, { status: 404 }); if (resource === "sites") await deleteNginxSite(String(existing[0].domain)); await db.delete(table).where(eq(table.id, numericId)); await audit(`${entity.singular} deleted`, firstLabel(entity, existing[0]), "", auth.user, getClientIp(req)); return Response.json({ ok: true });
