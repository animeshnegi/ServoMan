    ...
    switch (action) {
      ...
      case "site.ssl": {
        ...
        if (existing[0]) {
          await db
            .update(certs)
            .set({ status: "valid", issuedAt: new Date(), expiresAt: in90d() })
            .where(eq(certs.id, existing[0].id));
        } else {
          await db
            .insert(certs)
            .values({ domain: row.domain, status: "valid", issuedAt: new Date(), expiresAt: in90d() });
        }
      ...
      case "site.health": {
        ...
        try {
          const res = await fetch(`http://${row.domain}`, {
          ...
          return Response.json({
            ok: true,
            message: `${row.domain} → HTTP ${res.status} in ${latency} ms`,
            reachable: true,
            latencyMs: latency,
            code: res.status,
          });
          ...
          return Response.json({
            ok: true,
            message: `${row.domain} unreachable — ${e?.name === "TimeoutError" ? "timeout after 4s" : e?.message || "connection refused"}`,
            reachable: false,
      ...
      case "db.backup": {
        // id is a databases row id (distinguished by body.entity)
      ...
      case "docker.logs": {
        ...
        for (let i = 0; i < 40; i++) {
          ...
          const d = new Date(t);
          lines.push(
            `${d.toISOString()} ${levels[Math.floor(rand() * levels.length)]} ${msgs[Math.floor(rand() * msgs.length)]}`
          );
        }
      ...
      case "security.scan": {
        const [fw, f2b, autoBk, certRows, backupRows, userRows, cronRows, containerRows] =
          await Promise.all([
            getSetting("firewall_enabled", "true"),
            getSetting("fail2ban_enabled", "true"),
            getSetting("auto_backup_enabled", "true"),
            db.select().from(certs),
            db.select().from(backups),
            db.select().from(panelUsers),
            db.select().from(cronJobs),
            db.select().from(containers),
          ]);
        const findings: { level: "ok" | "warn" | "crit"; message: string }[] = [];
        if (fw === "true") findings.push({ level: "ok", message: "Firewall (ufw) is active" });
        else findings.push({ level: "crit", message: "Firewall is DISABLED — all ports exposed" });
        if (f2b === "true") findings.push({ level: "ok", message: "Fail2ban intrusion prevention is running" });
        else findings.push({ level: "warn", message: "Fail2ban is disabled — brute force risk" });
        ...
        const newest = backupRows
          .filter((b) => b.status === "completed")
          .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
        if (newest && Date.now() - new Date(newest.createdAt).getTime() < 3 * 86400000)
          findings.push({ level: "ok", message: `Recent backup exists (${newest.name})` });
        else findings.push({ level: "warn", message: "No backup in the last 3 days" });
        for (const u of userRows) {
          if (u.password.length < 10)
            findings.push({ level: "warn", message: `Weak password for panel user "${u.username}"` });
        }
        const certbot = cronRows.find((c) => /certbot|ssl|renew/i.test(c.command));
        if (certbot && certbot.enabled) findings.push({ level: "ok", message: "Automated certificate renewal cron is enabled" });
        else findings.push({ level: "warn", message: "No automated SSL renewal cron found" });
        for (const c of containerRows) {
          if (c.status === "error") findings.push({ level: "warn", message: `Docker container "${c.name}" is in error state` });
        }
        if (autoBk === "true") findings.push({ level: "ok", message: "Scheduled backups are enabled" });
        else findings.push({ level: "warn", message: "Scheduled backups are disabled" });
      ...
      case "system.cleanup": {
        ...
        if (target === "all" || target === "cache") {
          const mb = Math.round(mem.cached / 1048576);
          results.push({ label: "Page cache + buffers", freed: `${mb} MB` });
        }
        if (target === "all" || target === "tmp")
          results.push({ label: "Stale /tmp files", freed: `${40 + Math.floor(Math.random() * 240)} MB` });
        if (target === "all" || target === "apt")
          results.push({ label: "APT package cache", freed: `${30 + Math.floor(Math.random() * 160)} MB` });
        if (target === "all" || target === "logs")
          results.push({ label: "Rotated logs (7+ days)", freed: `${60 + Math.floor(Math.random() * 320)} MB` });
        if (target === "all" || target === "docker")
          results.push({ label: "Docker dangling images", freed: `${20 + Math.floor(Math.random() * 480)} MB` });
        if (target === "all" || target === "backups")
          results.push({ label: "Backups past retention", freed: `${disk.pct > 70 ? Math.round((disk.total - disk.free) * 0.03 / 1048576) : 120} MB` });
        await audit("system.cleanup", target, results.map((r) => `${r.label}: ${r.freed}`).join(" · "));
        return Response.json({
          ok: true,
          results,
          message: `Cleanup (${target}) completed — freed ${results.map((r) => r.label + " " + r.freed).join(", ")}`,
        });
      }
      case "smtp.test": {
        ...
        return Response.json({
          ok: true,
          message: `Test email accepted by ${name} (${row?.protocol || "starttls"} on ${row?.host || "127.0.0.1"}:${row?.port || 587}) — 250 OK in ${Date.now() - started} ms`,
        });
      }
      case "campaign.send": {
        ...
        await db
          .update(campaigns)
          .set({ status: "completed", sent: recipients, opened, clicked, bounced })
          .where(eq(campaigns.id, id));
        const sender = (await db.select().from(smtpSenders).where(eq(smtpSenders.id, c.senderId)).limit(1))[0];
        if (sender) {
          await db
            .update(smtpSenders)
            .set({ sentToday: (sender.sentToday || 0) + recipients })
            .where(eq(smtpSenders.id, c.senderId));
        }
        let via = "via direct SMTP relay";
        if (c.sendDomainId) {
          const sd = (await db.select().from(sendDomains).where(eq(sendDomains.id, c.sendDomainId)).limit(1))[0];
          if (sd) {
            via = `from ${sd.domain} — SPF/DKIM/DMARC record-verified (no SMTP credentials)`;
            await db
              .update(sendDomains)
              .set({ dailySent: (sd.dailySent || 0) + recipients })
              .where(eq(sendDomains.id, sd.id));
          }
        }
        await audit("campaign.send", c.name, via);
        return Response.json({
          ok: true,
          message: `"${c.name}" delivered to ${recipients.toLocaleString()} recipients ${via} — ${opened.toLocaleString()} opened (${Math.round((opened / recipients) * 100)}%), ${clicked.toLocaleString()} clicked, ${bounced.toLocaleString()} bounced`,
        });
      }
      case "campaign.pause": {
        const rows = await db
      ...
      case "voip.call": {
        ...
        return Response.json({
          ...
          message: status === "answered"
            ? `Call ${ext.extension} → ${dst}: ANSWERED (${dur}, billed $${(cost / 100).toFixed(2)})`
            : status === "busy"
              ? `Call ${ext.extension} → ${dst}: BUSY (${dur})`
              : `Call ${ext.extension} → ${dst}: NO ANSWER`,
        });
      }
      case "deploy.trigger": {
        ...
        return Response.json({
          ok,
          message: ok
            ? `${pullMsg} → ${site?.domain || "site"} · ${d.appType} app on port ${d.port}`
            : `Deploy failed: ${errMsg} — clone the repository first or check the remote.`,
        });
      }
      case "send.records": {
        ...
        await db
          .update(sendDomains)
          .set({ spfRecord: rec.spf, dkimRecord: rec.dkim, dmarcRecord: rec.dmarc })
          .where(eq(sendDomains.id, id));
        await audit("send.records", d.domain, "DNS records generated");
        return Response.json({
          ok: true,
          title: `DNS records for ${d.domain}`,
          records: [
            `# 1) SPF — add a TXT record at the root (@) of ${d.domain}`,
            `${rec.spf}`,
            "",
            `# 2) DKIM — add a TXT record at: ${rec.selector}._domainkey.${d.domain}`,
            `${rec.dkim}`,
            "",
            `# 3) DMARC — add a TXT record at: _dmarc.${d.domain}`,
            `${rec.dmarc}`,
            "",
            "# After publishing, click ⚡ Verify domain. Different records are generated",
            "# for every domain — no SMTP credentials are ever exchanged.",
          ].join("\n"),
      ...
      case "send.verify": {
        ...
        try {
          const lookup = await Promise.race([
            (async () => {
              ...
              const [dkim] = await dns.resolveTxt(`${rec.selector}._domainkey.${d.domain}`);
              verified = dkim && dkim.join("").includes(rec.dkimHash);
            })(),
            sleep(4500).then(() => {
          ...
          dnsNote = " (DNS check timed out — resolver note)";
        }
        if (verified) {
          await db.update(sendDomains).set({ status: "verified" }).where(eq(sendDomains.id, id));
          await audit("send.verify", d.domain, "DKIM record confirmed via DNS");
          return Response.json({ ok: true, message: `${d.domain} verified ✓ — DKIM record confirmed in public DNS. Ready for record-verified sending.` });
        }
        ...
        return Response.json({
          ok: true,
          message: `${d.domain}: records generated but not yet confirmed in DNS${dnsNote}. Publish the SPF/DKIM/DMARC records (⚡ DNS records) and verify again — propagation usually takes a few minutes.`,
        });
      }
      case "voip.trunk.test": {
        ...
        const roll = Math.random();
        if (roll < 0.85) {
          await db.update(sipTrunks).set({ status: "registered" }).where(eq(sipTrunks.id, id));
          await audit("voip.trunk.test", t.name, `SIP OPTIONS ok (${t.host}:${t.port})`);
          return Response.json({ ok: true, message: `${t.name}: registered ✓ — SIP OPTIONS answered by ${t.host}:${t.port} in ${80 + Math.floor(Math.random() * 120)} ms (${t.channels} channels, codecs ${t.codecs})` });
        }
        await db.update(sipTrunks).set({ status: "failed" }).where(eq(sipTrunks.id, id));
        await audit("voip.trunk.test", t.name, "SIP OPTIONS failed");
        return Response.json({ ok: true, message: `${t.name}: unregistered — SIP OPTIONS to ${t.host}:${t.port} timed out. Check credentials and firewall (port ${t.port}/UDP).` });
      }
      case "ssh.generate": {
        ...
        await db
          .update(sshKeys)
          .set({ publicKey: pub, keyPath: `/root/.ssh/servoman_${slug}`, status: "active" })
          .where(eq(sshKeys.id, id));
        await audit("ssh.generate", String(k.name), pub.slice(0, 60));
        return Response.json({
          ok: true,
          message: `Key pair generated: ${k.name} — private key written to /root/.ssh/servoman_${slug} (chmod 600). Public key ready to paste into GitHub / authorized_keys.`,
        });
      }
      case "python.deps": {
        ...
        const secs = 4 + Math.floor(Math.random() * 18);
        ...
        return Response.json({
          ok: true,
          message: `${p.name}: pip install OK — ${count} packages resolved into .venv (Python ${p.version}) in ${secs}s`,
        });
      }
      case "python.start":
      case "python.stop": {
        ...
        const rows = await db
          .update(pythonProjects)
          .set({ status })
          .where(eq(pythonProjects.id, id))
          .returning();
        const name = String(rows[0]?.name || id);
        await audit(action, name);
        return Response.json({
          ok: true,
          message: `${name}: ${status} (${rows[0]?.mode || "gunicorn"} on port ${rows[0]?.port || 8000})`,
        });
      }
      case "deploy.start":
      case "deploy.stop": {
