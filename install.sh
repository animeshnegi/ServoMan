...
# ServoMan performs privileged host operations through explicit adapters. Do not grant a general sudo shell.
cat > /etc/sudoers.d/servoman <<'EOF'
Defaults:servoman !requiretty
servoman ALL=(root) NOPASSWD: /usr/bin/systemctl, /bin/systemctl, /usr/sbin/ufw, /usr/bin/ufw, /usr/bin/certbot, /usr/bin/docker, /usr/bin/pg_dump, /usr/bin/pg_restore, /usr/bin/psql, /usr/bin/mysqldump, /usr/bin/mysql, /usr/bin/ssh-keygen, /usr/bin/shutdown, /usr/bin/tar, /usr/bin/stat, /usr/bin/mkdir, /usr/bin/chmod, /usr/bin/cat, /usr/bin/find, /usr/bin/journalctl, /usr/bin/apt-get, /usr/sbin/runuser, /usr/bin/runuser
EOF
