...
PANEL_PASS="$(head -c 24 /dev/urandom | base64 | tr -dc 'A-Za-z0-9' | head -c 16)"
PROXY_SECRET="$(head -c 48 /dev/urandom | base64 | tr -dc 'A-Za-z0-9' | head -c 48)"
REPO_DIR="$(pwd)"
...
mkdir -p /backups/servoman; chown root:root /backups/servoman; chmod 700 /backups/servoman
cat > /etc/sudoers.d/servoman <<'EOF'
Defaults:servoman !requiretty
servoman ALL=(root) NOPASSWD: /usr/bin/systemctl, /bin/systemctl, /usr/sbin/ufw, /usr/bin/ufw, /usr/bin/certbot, /snap/bin/certbot, /usr/bin/docker, /usr/bin/pg_dump, /usr/bin/pg_restore, /usr/bin/psql, /usr/bin/mysqldump, /usr/bin/mysql, /usr/bin/ssh-keygen, /usr/bin/shutdown, /usr/bin/tar, /usr/bin/stat, /usr/bin/mkdir, /usr/bin/chmod, /usr/bin/cat, /usr/bin/find, /usr/bin/journalctl, /usr/bin/apt-get, /usr/sbin/runuser, /usr/bin/runuser, /usr/sbin/nginx, /usr/bin/nginx, /bin/rm, /usr/bin/rm, /bin/ln, /usr/bin/ln, /usr/bin/install, /usr/bin/chown, /usr/bin/test
EOF
chmod 440 /etc/sudoers.d/servoman
visudo -cf /etc/sudoers.d/servoman >/dev/null
log "Building application"
cd "$PANEL_DIR"
sudo -u "$PANEL_USER" npm install --no-audit --no-fund
sudo -u "$PANEL_USER" npx drizzle-kit push || warn "drizzle push reported an issue"
sudo -u "$PANEL_USER" npx tsx src/db/seed.ts || warn "seed skipped"
...
EOF
sed -i "s/__SERVOMAN_PROXY_SECRET__/$PROXY_SECRET/" /etc/nginx/sites-available/servoman
...
ok "$PANEL_NAME $PANEL_VERSION installed"
ok "URL: http://$IP"
ok "Basic-auth user: servoman"
ok "Basic-auth password: $PANEL_PASS"
ok "Database password: $DB_PASS"
ok "Service: systemctl status servoman"
ok "Logs: journalctl -u servoman -f"
ok "The generated .env and passwords are stored on the server only."
