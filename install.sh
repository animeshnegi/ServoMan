...
[[ $EUID -eq 0 ]] || err "Run as root: sudo bash install.sh"; . /etc/os-release; [[ -f package.json ]] || err "Run from the ServoMan project directory."; export DEBIAN_FRONTEND=noninteractive
...
log "Installing OS packages"; apt-get update -yq; apt-get install -yq curl ca-certificates gnupg git unzip rsync build-essential python3 python3-dev python3-venv python3-pip nginx certbot python3-certbot-nginx php-fpm php-cli php-mbstring php-xml php-curl php-zip php-gd php-bcmath php-intl postgresql postgresql-contrib redis-server postfix fail2ban ufw apache2-utils sudo docker.io; systemctl enable --now docker >/dev/null 2>&1 || warn "Docker service could not be started"
...
log "Installing application"; rsync -a --delete --exclude node_modules --exclude .next --exclude .git "$REPO_DIR"/ "$PANEL_DIR"/
...
# Optional: a GitHub fine-grained token for repository/Actions/release management.
# SERVOMAN_GITHUB_TOKEN=
EOF
...
servoman ALL=(root) NOPASSWD: /usr/bin/systemctl, /bin/systemctl, /usr/sbin/ufw, /usr/bin/ufw, /usr/bin/certbot, /snap/bin/certbot, /usr/bin/docker, /usr/bin/pg_dump, /usr/bin/pg_restore, /usr/bin/psql, /usr/bin/createdb, /usr/bin/dropdb, /usr/bin/mysqldump, /usr/bin/mysql, /usr/bin/ssh-keygen, /usr/bin/shutdown, /usr/bin/tar, /usr/bin/stat, /usr/bin/mkdir, /usr/bin/chmod, /usr/bin/cat, /usr/bin/find, /usr/bin/journalctl, /usr/bin/apt-get, /usr/sbin/runuser, /usr/bin/runuser, /usr/sbin/nginx, /usr/bin/nginx, /bin/rm, /usr/bin/rm, /bin/ln, /usr/bin/ln, /usr/bin/install, /usr/bin/chown, /usr/bin/test, /usr/sbin/asterisk, /usr/bin/asterisk, /usr/bin/python3.10, /usr/bin/python3.11, /usr/bin/python3.12
EOF
chmod 440 /etc/sudoers.d/servoman; visudo -cf /etc/sudoers.d/servoman >/dev/null
log "Building application"; cd "$PANEL_DIR"; sudo -u "$PANEL_USER" npm install --no-audit --no-fund; sudo -u "$PANEL_USER" npx drizzle-kit push || warn "drizzle push reported an issue"; sudo -u "$PANEL_USER" npx tsx src/db/seed.ts || warn "seed skipped"; sudo -u "$PANEL_USER" npm run build
...
log "Configuring authenticated reverse proxy"; htpasswd -bc /etc/nginx/.servoman_admin servoman "$PANEL_PASS" >/dev/null
...
log "Configuring firewall"; ufw allow 22/tcp >/dev/null 2>&1 || true; ufw allow 80/tcp >/dev/null 2>&1 || true; ufw allow 443/tcp >/dev/null 2>&1 || true
...
IP="$(hostname -I 2>/dev/null | awk '{print $1}')"; [[ -n "$IP" ]] || IP="<server-ip>"; ok "$PANEL_NAME $PANEL_VERSION installed"; ok "URL: http://$IP"; ok "Basic-auth user: servoman"; ok "Basic-auth password: $PANEL_PASS"; ok "Database password: $DB_PASS"; ok "Python 3.x project manager: Flask / Django / FastAPI with per-app venv + systemd"; ok "GitHub management: set SERVOMAN_GITHUB_TOKEN in $PANEL_DIR/.env"; ok "Service: systemctl status servoman"; ok "Logs: journalctl -u servoman -f"; ok "The generated .env and passwords are stored on the server only."
